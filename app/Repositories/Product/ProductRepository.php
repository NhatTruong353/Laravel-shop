...
class ProductRepository extends BaseRepository implements ProductRepositoryInterface
{
    ...
    public function getProductsOnIndexAdmin($request){
        $search = $request->search ?? '';
        $product = $this->model->where('name','like','%' . $search . '%')->orderby('id','desc');
        $product =$this->searchAndPaginationAdmin($product);
        $product->appends(['search' => $search]);
        return $product;
    }
    private function searchAndPaginationAdmin($product)
    {
        $perPage =  5;
