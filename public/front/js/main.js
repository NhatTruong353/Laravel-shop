...
function addWish(productId) {
    $.ajax({
        ...
        success: function (response){
            ...
            var cartHover_tbody = $('.select-items tbody');
            var cartHover_existItem = cartHover_tbody.find("tr"+"[data-rowId='" + response['wishlist'].rowId + "']");
            ...
            if(cartHover_existItem.length){
                ...
                var newItem =
                    ...
                    '    <td class="si-pic"><img style="height: 70px" src="front/img/products/' + response['wishlist'].options.images[0].path + '"></td>\n' +
                    '    <td class="si-text">\n' +
                    '        <div class="product-selected">\n' +
                    '            <h6>' + response['wishlist'].name + '</h6>\n' +
                    '        </div>\n' +
                    '    </td>\n' +
                    '    <td class="so-close">\n' +
                    '        <i onclick="removeCart(\'' + response['wishlist'].rowId + '\')" class="ti-close" ></i>\n' +
                    '    </td>\n' +
                    '</tr>';
            ...
            alert('Add successfull! \nProduct: ' + response['wishlist'].name + ' to wishlist')
...
function addCartInDetails(productId) {
    ...
    if(qty <= qty_fill){
        ...
        alert('the number of products is not enough to add');
    }
        ...
        $.ajax({
            ...
            success:function (result){
                if(result.status==='Success'){
                    ...
                    $('#coupon_code_msg').html('Please enter valid coupon code');
                }
